...
- 2026-09-14 実機確認の反映(第2回): 冒頭の飾りの図と四つの顔の図を廃止。部屋の図にワット数を持たせて足元の節に移し、分電盤の図と統合。境目の図を壁の断面に作り直し。段落を消せる図(分電盤、電圧の階段、お金の流れ)は消した段落の位置に置き、対応する文章を削った。エジソンは既存のガイド画像を文章の脇に小さく置く(認識のための図)
- 図の置き方の規則: 段落の代わりになる図は、その段落があった位置に全幅で置く。見出しの直後に機械的に置かない。人物の認識のための図は文章の脇に小さく置く。飾りの図は置かない
